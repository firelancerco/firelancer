 ...
 * ### Preview mode
 *
 * The `mode` parameter can be either `crop` or `resize`.
 *
 * ### Focal point
 *
 * When cropping an image (`mode=crop`), Firelancer will attempt to keep the most "interesting" area of the image in the cropped frame. It does this
 ...
 * ### Format
 *
 * the image format can be specified by adding the `format` query parameter:
 *
 ...
 * * `avif`
 *
 * The `format` parameter can also be combined with presets.
 *
 * ### Quality
 *
 * the image quality can be specified by adding the `q` query parameter:
 *
 ...
 *
 * The `q` parameter can also be combined with presets.
 *
 * ### Transform presets
 *
 ...
 * configured via the AssetServerOptions.
 *
