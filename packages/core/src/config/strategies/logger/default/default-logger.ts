 ...
 * default Firelancer configuration, you do not need to specify it explicitly in your server config. You would only need
 * to specify it if you wish to change the log level (which defaults to `LogLevel.Info`) or remove the timestamp.
 *
 * @example
 * ```ts
 * import { DefaultLogger, LogLevel, FirelancerConfig } from '\@firelancerco/core';
 *
 * export config: FirelancerConfig = {
 *     // ...
 *     logger: new DefaultLogger({ level: LogLevel.Debug, timestamp: false }),
 * }
 * ```
 */
export class DefaultLogger implements FirelancerLogger {
    ...
    error(message: string, context?: string, trace?: string | undefined): void {
        ...
        if (this.level >= LogLevel.Error) {
            this.logMessage(pc.red('error'), pc.red(this.ensureString(message) + (trace ? `\n${trace}` : '')), context);
        }
    }
