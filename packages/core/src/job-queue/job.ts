...
export class Job<T extends JobData<T> = any> {
    ...
    start() {
        if (this._state === JobState.PENDING || this._state === JobState.RETRYING) {
            ...
            this._attempts++;
            Logger.debug(
                `Job ${this.id?.toString() ?? 'null'} [${this.queueName}] starting (attempt ${this._attempts} of ${this.retries + 1})`,
            );
        }
    }
    ...
    fail(err?: any) {
        ...
        if (this.retries >= this._attempts) {
            ...
            Logger.warn(
                `Job ${this.id?.toString() ?? 'null'} [${this.queueName}] failed (attempt ${this._attempts} of ${this.retries + 1})`,
            );
        } else {
