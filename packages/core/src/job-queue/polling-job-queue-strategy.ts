...
/**
 * @description
 * Defines the backoff strategy used when retrying failed jobs. Returns the delay in ms
 * that should pass before the failed job is retried.
 */
...
export abstract class PollingJobQueueStrategy extends InjectableJobQueueStrategy {
    ...
    async start<Data extends JobData<Data> = object>(queueName: string, process: (job: Job<Data>) => Promise<any>) {
    ...
    async stop<Data extends JobData<Data> = object>(queueName: string, process: (job: Job<Data>) => Promise<any>) {
