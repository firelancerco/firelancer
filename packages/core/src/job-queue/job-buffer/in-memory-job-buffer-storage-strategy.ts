...
/**
 * @description
 * A JobBufferStorageStrategy which keeps the buffered jobs in memory. Should
 * _not_ be used in production, since it will lose data in the event of the server stopping.
 *
 * Instead, use the DefaultJobQueuePlugin with the `useDatabaseForBuffer: true` option set,
 * or theBullMQJobQueuePlugin or another custom strategy with persistent storage.
 */
