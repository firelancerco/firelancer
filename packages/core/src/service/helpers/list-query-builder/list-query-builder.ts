 ...
 */
export type ExtendedListQueryOptions<T extends FirelancerEntity> = {
    relations?: string[];
 ...
 *
 * ```
 * When Firelancer bootstraps, it will find the `BlogPostListOptions` input and, because it is used in a query
 ...
 *
 * Your controller function will then look like this:
 *
 * ```ts
 * \@Controller()
...
export class ListQueryBuilder implements OnApplicationBootstrap {
     ...
     */
    filterObjectHasProperty<FP extends FilterParameter<FirelancerEntity>>(
     ...
     */
    build<T extends FirelancerEntity>(
        entity: Type<T>,
    ...
    private addNestedWhereClause(qb: WhereExpressionBuilder, whereGroup: WhereGroup, parentOperator: LogicalOperator) {
     ...
     * method.
     */
    private getMinimumRequiredRelations<T extends FirelancerEntity>(
    ...
    private isPropertyUsedInFilter(property: string, filter?: NullOptionals<FilterParameter<any>> | null): boolean {
        return !!(
            filter &&
            (filter[property] ||
     ...
     */
    private joinCalculatedColumnRelations<T extends FirelancerEntity>(
        qb: SelectQueryBuilder<T>,
        entity: Type<T>,
        options: ListQueryOptions<T>,
    ) {
        const calculatedColumns = getCalculatedColumns(entity);
        const filterAndSortFields = unique([...Object.keys(options.filter || {}), ...Object.keys(options.sort || {})]);
            ...
            if (instruction) {
                ...
                for (const relation of relations) {
                    ...
                    if (!relationIsAlreadyJoined) {
                        ...
                        const relationAlias = relation.includes('.') ? relation.split('.').reverse()[0] : relation;
    ...
    private isRelationAlreadyJoined<T extends FirelancerEntity>(qb: SelectQueryBuilder<T>, alias: string): boolean {
