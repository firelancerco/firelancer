...
import { CacheStrategy, Logger, ConfigService, SetCacheKeyOptions } from '../config';
...
export class CacheService {
     ...
     */
    async set<T extends JsonCompatible<T>>(key: string, value: T, options?: SetCacheKeyOptions): Promise<void> {
        try {
    ...
    async invalidateTags(tags: string[]): Promise<void> {
        try {
            ...
            Logger.error(`Could not invalidate tags [${tags.join(', ')}] from CacheService`, undefined, e.stack);
        }
    }
}
