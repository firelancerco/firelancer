...
import { EventBus, Injector, Logger, RequestContext } from '@firelancerco/core';
...
import { EmailDetails, EmailTransportOptions, InitializedEmailPluginOptions, IntermediateEmailDetails } from './types';
...
export class EmailProcessor {
    ...
    async init() {
        ...
        this.generator = this.options.emailGenerator ? this.options.emailGenerator : new HandlebarsMjmlGenerator();
    ...
    async process(data: IntermediateEmailDetails) {
        ...
        try {
            const bodySource = await this.options.templateLoader.loadTemplate(new Injector(this.moduleRef), ctx, {
