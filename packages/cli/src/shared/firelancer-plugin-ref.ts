...
export class FirelancerPluginRef {
    ...
    getMetadataOptions() {
        const pluginDecorator = this.classDeclaration.getDecorator('FirelancerPlugin');
        if (!pluginDecorator) {
            throw new Error('Could not find FirelancerPlugin decorator');
        }
        const pluginOptions = pluginDecorator.getArguments()[0];
        if (!pluginOptions || !Node.isObjectLiteralExpression(pluginOptions)) {
            throw new Error('Could not find FirelancerPlugin options');
        }
    ...
    addAdminApiExtensions(extension: { schema: VariableDeclaration | undefined; resolvers: ClassDeclaration[] }) {
        const pluginOptions = this.getMetadataOptions();
