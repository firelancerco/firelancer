...
export class PackageJson {
    private _firelancerPackageJsonPath: string | undefined;
    private _rootPackageJsonPath: string | undefined;
    constructor(private readonly project: Project) {}

    get firelancerPackageJsonPath() {
        return this.locatePackageJsonWithFirelancerDependency();
    }
    ...
    determineFirelancerVersion(): string | undefined {
        const packageJson = this.getPackageJsonContent();
        return packageJson.dependencies['@firelancerco/core'];
    }
    ...
    addScript(scriptName: string, script: string) {
        ...
        const packageJsonPath = this.firelancerPackageJsonPath;
        if (packageJsonPath) {
    ...
    locatePackageJsonWithFirelancerDependency() {
        if (this._firelancerPackageJsonPath) {
            return this._firelancerPackageJsonPath;
        }
        const rootDir = this.getPackageRootDir().getPath();
        ...
        if (this.hasFirelancerDependency(rootPackageJsonPath)) {
            return rootPackageJsonPath;
        }
        for (const dir of potentialMonorepoDirs) {
            ...
            for (const subDir of fs.readdirSync(monorepoDir)) {
                ...
                if (this.hasFirelancerDependency(packageJsonPath)) {
                    this._firelancerPackageJsonPath = packageJsonPath;
    ...
    private hasFirelancerDependency(packageJsonPath: string) {
